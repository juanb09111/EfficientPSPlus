...
#SBATCH --output=res/inference_res_%a.txt
#SBATCH --error=res/inference_err_%a.txt
#
#!/bin/bash
...
module load gcc/9.1.0 cuda/11.1.0
...
python -u inference.py --model_name=EfficientPS --config=configs/effps.yml --nodes=1 --ngpus=1 --ip_adress $ip1
